...
class VideoGenerateRouteTest {
    ...
    @Test
    fun `summarize - returns 200 with summary on success`() = withVideoRouteApp { client ->
        ...
        val response = client.post("/v1/summarize") {
        ...
        verify(exactly = 1) { VimeoIdExtractor.extractId(rawVideoId) }
        coVerify(exactly = 1) { summarizeVideoUseCase.execute(vimeoId, prompt) }
    }

    @Test
    fun `summarize - returns 200 with null summary when use case returns null`() = withVideoRouteApp { client ->
    ...
    @Test
    fun `summarize - sends null prompt to use case when prompt is omitted`() = withVideoRouteApp { client ->
    ...
    @Test
    fun `summarize - returns 500 when VimeoIdExtractor throws`() = withVideoRouteApp { client ->
    ...
    @Test
    fun `summarize - returns 500 when use case throws`() = withVideoRouteApp { client ->
    ...
    @Test
    fun `summarize multiple - returns 200 with combined summary on success`() = withVideoRouteApp { client ->
        ...
        val response = client.post("/v1/summarize/multiple") {
            contentType(ContentType.Application.Json)
        ...
        verify(exactly = 0) { VimeoIdExtractor.extractId(any()) }
    }

    @Test
    fun `summarize multiple - passes null prompt when omitted`() = withVideoRouteApp { client ->
    ...
    @Test
    fun `summarize multiple - returns 200 with empty list`() = withVideoRouteApp { client ->
    ...
    @Test
    fun `summarize multiple - returns 500 when use case throws`() = withVideoRouteApp { client ->
    ...
    @Test
    fun `summarize multiple - error body contains error key`() = withVideoRouteApp { client ->
